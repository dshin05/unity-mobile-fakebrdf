...
namespace mset {
	...
	public class Util
	{
		...
		public static bool makeReadable( ref Texture tex ) {
			...
			TextureImporter ti = getTextureImporter(path,"generic Util.makeReadable");
		...
		public static bool makeCubeReadable( ref Cubemap cube ) {
			...
			TextureImporter ti = getTextureImporter(path, "Util.makeCubeReadable");
			if( ti == null ) return false;
			ti.isReadable = true;
			AssetDatabase.ImportAsset(path);
		...
		public static bool makeCubeFrom2D( String path, bool mipmap, int maxSize ) {
			TextureImporter ti = getTextureImporter(path, "Util.makeCubeFrom2D");
			ti.isReadable = true;
		...
		public static bool resizeCube( ref Cubemap cube, int maxSize ) {
			string path = AssetDatabase.GetAssetPath(cube);
			TextureImporter ti = getTextureImporter(path, "Util.resizeCube");
			if( ti == null ) return false;
			ti.maxTextureSize = maxSize;
			AssetDatabase.ImportAsset(path);
			cube = (Cubemap)AssetDatabase.LoadAssetAtPath(path,typeof(Cubemap));
		...
		public static bool makeReflection( string path ) {
			TextureImporter ti = getTextureImporter(path,"Util.makeReflection");	
			if( ti == null ) return false;
		...
		*/
		...
		public static bool hasAlpha(Texture2D tex) {
			...
			TextureImporter ti = Util.getTextureImporter(AssetDatabase.GetAssetPath(tex), "Util.hasAlpha");
